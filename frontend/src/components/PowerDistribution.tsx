...
export function PowerDistribution() {
  ...
  // Power consumers from element tree
  const consumers: { name: string; power_w: number; rail: string; domain: string }[] = []
  for (const el of elements.values()) {
    if (el.power_avg_w && el.power_avg_w > 0 && el.element_type === 'subsystem') {
      consumers.push({
        name: el.name,
        power_w: el.power_avg_w * el.quantity,
  ...
  return (
    <div style={{ padding: '1rem', overflowY: 'auto', height: '100%' }}>
      ...
      <p style={{ fontSize: '0.72rem', color: '#9ca3af', marginBottom: '0.75rem' }}>
        Generation, storage, and distribution with duty cycles and margins.
      </p>
      ...
      {/* Duty cycle table */}
      <div className="card">
        <h3 style={{ fontSize: '0.82rem', marginBottom: '0.3rem' }}>Power by Mode</h3>
        <SVGBarChart
          data={[
