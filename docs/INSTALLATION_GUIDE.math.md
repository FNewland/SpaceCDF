...
### 8.1 Run with PostgreSQL instead of in-memory storage

By default SpaceCDF keeps studies in memory only — they vanish when you
restart the server.  To persist them, install PostgreSQL 15+ and add a
database URL to your environment:

```bash
export SPACECDF_DATABASE_URL=postgresql://localhost/spacecdf
```

PostgreSQL adds about 200 MB to disk and ~100 MB to RAM.

### 8.2 Remote access via Tailscale
...
### 8.3 Keep the ECSS standards library offline
