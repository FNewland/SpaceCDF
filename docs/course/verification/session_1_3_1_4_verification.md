...
**Claim:** Total_Score = ?(Weight × NormScore) / ?Weight
...
**Claim:** R = R_E × [?((h/R_E + 1)^2 - cos^2(?)) - sin(?)]
...
- R_E = Earth radius, h = altitude, ? = elevation angle
- Law of sines gives: R/sin(90°+?) = (R_E+h)/sin(central_angle)
...
#### 6. SA Power Formula
**Claim:** P_SA = P_max_sunlight + (P_eclipse × t_eclipse) / (t_sunlight × eta_charge)
...
T = 2pi?(a^3/mu) where a = (6371+500)×1000 = 6.871×10? m, mu = 3.986×10¹? m^3/s^2
T = 2pi?((6.871×10?)^3 / 3.986×10¹?) = 2pi?(3.244×10^2? / 3.986×10¹?) = 2pi×28520 = 5693 s = 94.9 min ~ 95 min Y
**Confidence:** HIGH (computed)

#### 10. Eclipse Fraction for 500 km SSO
**Claim:** ~35% eclipse fraction.
**Verification:** APPROXIMATELY CORRECT. For a circular 500 km orbit, the eclipse fraction depends on the beta angle (angle between orbit plane and sun direction). For SSO, beta angle varies seasonally. Maximum eclipse fraction occurs near solstice: f_eclipse ~ arccos(?(1-(R_E/(R_E+h))^2))/pi. For h=500 km: f = arccos(?(1-(6371/6871)^2))/pi = arccos(0.375)/pi ~ 0.38 (38%). Annual average is ~33-35%.
